...
@log_connector
async def get_async_openai_client(settings: FastLangFrameSettings = None) -> AsyncOpenAI:
...
@log_connector
def get_langchain_chat_model(model_name: str = None) -> ChatOpenAI:
    """Returns a Langchain Chat Model (OpenAI or Azure)"""
    settings = get_settings()
    
    # Azure OpenAI logic
    if settings.azure_openai_api_key and settings.azure_openai_endpoint:
        deployment = settings.azure_deployment_name or model_name or settings.model_name
