...
def login_with_password(username: str, password: str) -> Optional[Dict[str, Any]]:
    ...
    # We can assume the backend is at ZITADEL_URL but on the FastAPI port (usually 8000)
    # or just use a new setting if needed. For now, let's use a common pattern.
    backend_url = settings.zitadel_url.replace(":8080", ":8000") if settings.zitadel_url else "http://localhost:8000"
...
def check_auth():
    ...
    if settings.jwt_secret_key == "your_jwt_secret_key" and not settings.zitadel_url:
        return True
