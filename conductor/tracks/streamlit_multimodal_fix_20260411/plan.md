...
- [ ] Task: If the dependency is missing, add `st_chat_input_multimodal` to the `pyproject.toml` used by the templates.
- [ ] Task: Ensure that running the Streamlit app with `poetry run streamlit run chat_test/app.py` within the generated project's virtual environment resolves the issue.
...
- [ ] Task: Review the core `README.md` and any relevant guides to ensure that the instruction for starting the Streamlit app explicitly uses `poetry run streamlit run chat_test/app.py`.
