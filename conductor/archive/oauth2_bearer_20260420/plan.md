...
## Phase 3: Configure Swagger UI OAuth2 Flow [checkpoint: 2478d13]
...
- [x] Task: Configure the FastAPI app in `src/core/server.py` with `swagger_ui_init_oauth` and set up the OAuth2 scheme pointing to Authelia's `/api/oidc/authorization` and `/api/oidc/token` endpoints (Green Phase). (5cfd643)
- [x] Task: Conductor - User Manual Verification 'Phase 3: Configure Swagger UI OAuth2 Flow' (Protocol in workflow.md) (2478d13)